...
const positiveIntegerEffect = (name: string, value: number): Effect.Effect<number, Error> => {
  if (Number.isSafeInteger(value) && value > 0) return Effect.succeed(value)
  return Effect.fail(new Error(`${name} requires a positive integer`))
}
...
const searchCommand = Command.make("search", {
  ...
  positiveIntegerEffect("--max", config.max).pipe(
    Effect.flatMap((max) => runCommand({ tag: "search", query: config.query, source: config.source, max, sort: config.sort }))
  )).pipe(Command.withShortDescription("Search papers by keyword"))
...
const refsCommand = Command.make("refs", {
  ...
  max: Flag.integer("max").pipe(Flag.withDefault(DEFAULT_MAX), Flag.withDescription("Maximum references")),
  json: Flag.boolean("json").pipe(Flag.withDescription("Emit raw JSON"))
}, (config) =>
  Effect.gen(function*() {
    const id = yield* parseIdentifierEffect("refs", config.id)
    const max = yield* positiveIntegerEffect("--max", config.max)
    yield* runCommand({ tag: "refs", id, max, json: config.json })
